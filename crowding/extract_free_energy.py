...
def process_directory(directory_path, box_size, probing_radius, tmax, shift):
    xyz_files = [f for f in os.listdir(directory_path) if f.endswith('.xyz')]
    if not xyz_files:
        print(f"No .xyz files found in {directory_path}")
        return None
    ...
    for xyz_file in xyz_files:
        file_path = os.path.join(directory_path, xyz_file)
        print(f"Processing {xyz_file}...")
        widom = WidomsMethod(file_path, box_size, tmax)
        profile = widom.compute_profile(probing_radius, shift)
        all_profiles.append(profile)
...
def main():
    ...
    for probing_radius in np.arange(0.0, 3.1, 0.1):
        shift_den = 0.0
        shift_dil = 33.0
        average_profile_den = process_directory(args.directory_path, box_size, probing_radius, tmax, shift_den)
        average_profile_dil = process_directory(args.directory_path, box_size, probing_radius, tmax, shift_dil)
        average_profile_array.append(average_profile_den / average_profile_dil)

    average_profile_array = np.array(average_profile_array)
    output_file = f'par_prob_distr_sp{args.output_suffix}_crowd.dat'
