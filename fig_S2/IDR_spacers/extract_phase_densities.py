...
# Constants
RADIUS = 0.3
HEIGHT = RADIUS - 0.38 / 2
V1 = np.pi / 3 * (3 * RADIUS - HEIGHT) * HEIGHT**2
V2 = 4 * np.pi / 3 * RADIUS**3 - 2 * V1
...
def plot_profile(replica_data, ax):
    ...
    density1, density2 = np.mean(y_data_avg[:4]), np.mean(y_data_avg[21:25])
...
def analyze_density_profiles():
    ...
    temperatures = [290, 295, 305, 310, 312]
